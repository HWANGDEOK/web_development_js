...
-> 유효성 검사를 필수적으로 수행하여 정상일 경우에만 그 다음 로직을 실행할 수 있어야 함.
...
그렇다면 SCISSORS, ROCK, PAPER로 바뀌게 될텐데 한국인 개발자 입장에서는 오히려 번거롭고 알아보기 힘들 것 같습니다. 그럼에도 불구하고 자주 사용되는 문자열을 상수로 교체하는 이유는 다음과 같습니다.
...
#### 이벤트 핸들러를 onclick에 모두 작성했을 때의 비효울성을 확인한 결과 개발자들에게 필요한 것은?
...
다만 평균을 구할 때 매번 여러분들이 (a+b) / 2로 작성해뒀다면, 갑자기 3으로 나눠야 할 필요가 있을 때 ctrl + h을 눌러가면서 하나하나 바꾼 후에 코드가 젣대로 동작하는지 테스트 해야 한다면,
...
하지만 할당된 값이 없는 num2의 경우 undefined로 남아있게 되고, num1+num2 = 1+undefined가 되기 때문에 그 연산 결과 NaN(Not-a-Number)가 됩니다.
...
많은 것을 했는데,
if 로 작성 / switch로 개선 / GUI 형태로 나가서 html과 연결시키는 것까지 완료.
