...
// 새로운 학생을 추가하는 함수
function addStudents(name, age, gender, grade){
  const newStudent ={
...
function getTopStudents(){
  ...
  // }
    ...
    console.log(`이름 : ${topStudent.name}\n성적 : ${topStudent.grade}점`)
  );
}

// 이상의 getTopStudents()함수의 경우 출력 결과가 id부터 성별 및 성적까지 전부 출려되기 때문에 가독성이 좀 떨어져 보이고, 이미 printStudents()를 통해서 객체를 출력하는 .forEach()를 사용해봤기 때문에
// 실행 예
// 이름 : 김일
// 성적 : 85점
...
// 객체 생성
addStudents("김일", 20, "남", 85);
addStudents("김이", 21, "여", 78);
addStudents("김삼", 22, "여", 100);
addStudents("김사", 23, "남", 55);
